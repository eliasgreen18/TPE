...
\begin{document}
	...
	\section{Problemas}
    	...
    	\begin{proc}{esPeriodico}{\In t: $toroide$, \Inout p: \ent, \Out result: \bool}{}
    		...
    		\post{result = \True \iff (\exists k: \ent)(k > 0 \land toroideModificadoMultiplesVeces(t,t,k) \land \paraTodo{j}{(1}{k} \implicaLuego \neg (toroideModificadoMultiplesVeces(t,t,j)) \land p = k)) } 
    	\end{proc} \\
    	...
    	\begin{proc}{primosLejanos}{\In t1: $toroide$, \In t2: $toroide$, out primos: \bool}{}
    		...
    		\post{primos = \True \iff (\exists k:\ent)(k>0 \land (toroideModificadoMultiplesVeces(t1, t2, k) \lor toroideModificadoMultiplesVeces(t2, t1 , k) ))}
    	\end{proc} \\
    	...
    	\begin{proc}{seleccionNatural}{\In ts: \TLista{$toroide$}, \Out res: \ent}{}
    		\pre{todosValidos(ts)}
    		\post{enRango(res, ts) \yLuego pertenece(ts, ts[res]) \land ((\exists i:\ent)(0 \leq i < \longitud{ts} \yLuego (toroideInmortal(ts[i]) \land res = i)) \lor \paraTodo{j}{(0}{\longitud{ts} \implicaLuego (\exists k: \ent) (k>0 \yLuego TicksParaMorir(t[j], k) \land (\exists h : \ent)(TicksParaMorir(t[res]), h \land h \leq k))))}}
    	\end{proc} \\
    	...
    	\begin{proc}{fusionar}{\In t1: $toroide$, \In t2: $toroide$, \Out res: $toroide$}{}
    		...
    		\post{(\forall i:\ent)(enRango(i,t1) \implicaLuego (\forall j:\ent)(enRango(j,t1[0]) \implicaLuego (estaViva(res,i,j)=(estaViva(t1,i,j)) \land estaViva(t2,i,j))))}
    	\end{proc} \\
	    ...
	    \begin{proc}{vistaTrasladada}{\In t1: $toroide$, \In t2: $toroide$, \Out res: \bool}{}
    		\pre{valido(t1) \land valido(t2) \land mismasDimensiones(t1,t2)}
    		\post{res = \True \iff (\forall i : \ent)(enRango(i,t1) \implicaLuego (\forall j : \ent)(enRango(j,t1[0]) \implicaLuego (\exists a : \ent)(enRango(a,t1) \yLuego (\exists b : \ent)(enRango(b,t1[0]) \yLuego (estaViva(t1,i,j)=estaViva(t2,corregirPosicionFila(i+a),corregirPosicionColumna(j+b)) ) ) ) ) )}
    	\end{proc} \\
	...
	\section{Auxiliares}
    	...
    	\subsection{Predicados} \vspace{1em}
        	...
        	\auxpred{toroideModificadoUnTick}{t1: $toroide$, t2: $toroide$}{(\forall i:\ent)(enRango(i,t) \implicaLuego (\forall j:\ent) (enRango(j,t[0]) \implicaLuego t2[i][j]=posicionModificada(t1, (i, j))))} \\
        	...
        	\auxpred{toroideInmortal}{t : $toroide$}{\exists tm:$toroide$)((mismasDimensiones(t, tm) \land tableroMuerto(tm)) \yLuego (\forall k:\ent) (k>0 \implicaLuego \neg toroideModificadoMultiplesVeces(t, tm, k))} \\
        	...
        	\auxpred{pertenece}{ts:\TLista{$toroide$}, t: $toroides$}{(\exists i: \ent)(enRango(i, ts) \yLuego t=ts[i]))}
        	
        	\auxpred{tableroVivo}{t:$toroide$}{(\exists i:\ent)(enRango(i,t) \yLuego (\exists j:\ent)(enRango(j,t[0]) \yLuego estaViva(t,i,j) ) )} \\
	
	        \auxpred{tableroMuerto}{t:$toroide$}{\paraTodo{i}{0}{\longitud{t} \implicaLuego \paraTodo{j}{0}{\longitud{t[0]}} \implicaLuego t[i][j]= \False}}
